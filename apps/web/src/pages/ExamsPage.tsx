...
import { useExams, useSaveExam, useDeleteExam } from '../features/exams/useExams'
...
function ExamForm({ close, save }: { close: () => void; save: (exam: Exam) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!group) return
    const activeStudents = students.filter((student) => student.status === 'active' && student.groups.includes(group.name))
    const exam: Exam = {
      id: `e-${Date.now()}`,
      groupId: group.id,
      ...
      results: activeStudents.map((student, index) => ({
      ...
      <section className="teacher-modal group-modal" role="dialog" aria-modal="true">
        <header>
          <div>
            ...
            <p>Guruh tanlang, keyin ballar jadvali avtomatik ochiladi</p>
          </div>
          <button type="button" onClick={close} aria-label="Yopish"><X size={18} /></button>
        </header>
        <form onSubmit={submit}>
          <div className="form-grid">
            <label className="form-wide">Nomi<input name="name" required placeholder="Masalan: Mock IELTS Reading" /></label>
            <label>Guruh<select name="groupId" required defaultValue={groups[0]?.id ?? ''}>{groups.filter((group) => group.active).map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label>
...
export function ExamsPage() {
  ...
  const saveMutation = useSaveExam()
  ...
  async function createExam(exam: Exam) {
    const saved = await saveMutation.mutateAsync(exam)
  ...
  return (
    <section className="exams-page">
      <div className="page-heading">
        <div>
          <h1>Imtihonlar</h1>
          <p>Guruh bo‘yicha ball kiritish, reyting va natijalar arxivi</p>
        </div>
      ...
      {creating && <ExamForm close={() => setCreating(false)} save={(exam) => void createExam(exam)} />}
        ...
        <div className="exams-layout">
                ...
                return (
                  <div key={exam.id} className={`exam-archive-item ${exam.id === activeExam.id ? 'active' : ''}`}>
                    <button type="button" onClick={() => requestExamChange(exam.id)}>
                      ...
                      <span><CalendarDays size={13} />{exam.date}</span>
          ...
          <div className="exams-main">
            ...
            <section className="panel finance-table exams-table">
              ...
              {(isDirty || saveMutation.isPending || saveFeedback !== 'idle') && (
                <footer className="exam-results-actions" aria-live="polite">
                  ...
                  {saveFeedback !== 'success' && (
                    <button className="finance-primary-action" type="button" onClick={() => void saveResults()} disabled={saveMutation.isPending || !isDirty}>
                      <Save size={15} />{saveMutation.isPending ? 'Saqlanmoqda...' : 'Natijalarni saqlash'}
                    </button>
                  )}
                </footer>
              )}
