...
export function useSaveTeacherAttendance() { const client = useQueryClient(); return useMutation({ mutationFn: (session: AttendanceSession) => teacherAttendanceRepository.save(session), onSuccess: async (session) => Promise.all([client.invalidateQueries({ queryKey: ['teacher-attendance', session.groupId, session.date] }), client.invalidateQueries({ queryKey: ['teacher-rating'] })]) }) }
